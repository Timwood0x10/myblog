...
flowchart LR
    ...
    C2 --> A
    C3 --> A
    A --> B["Best Child"] --> R["Replace Parent"]

    classDef default fill:#1e1e2e,stroke:#e0af68
    classDef output fill:#1e1e2e,stroke:#50fa7b
    class P,M,C1,C2,C3,A default
    class B,R output
```
...
```mermaid
flowchart TD
    API["api/evolution/ — Public API\nPopulation, DreamCycle,\nMutator, Promoter interfaces"]

    subgraph CORE["internal/ares_evolution/ — Core GA Engine"]
        direction LR
        M["mutation\n.Strategy / .Mutator / .Types"] --> G["genome\n.Population\n.Selection[] / .Crossover[]"]
        G --> SC["scheduler\n.Scheduler / .DreamCycle"]
        SC --> EX["experience\n.Evidence / .Store / .Guidance"]

        W["WiredEvolutionSystem\nFactory + Adapter — wires\nScheduler, Population, Scoring, Genealogy"]
    end

    subgraph RUN["internal/evolution/ — Runtime Evolution Engine"]
        direction LR
        GR["genome/Registry\nMemoryGenome / PlannerGenome"] --> DR["diff/Registry\nDiffAll()"]
        DR --> CR["coordinator/Coordinator\nApply / Reject / Delay"]
    end

    API --> CORE
    CORE -->|"Phase 6 bridge"| RUN

    classDef default fill:#1e1e2e,stroke:#e0af68
    classDef secondary fill:#1e1e2e,stroke:#56b6c2
    classDef tertiary fill:#1e1e2e,stroke:#bd93f9
    classDef output fill:#1e1e2e,stroke:#50fa7b
    class API default
    class M,G,SC,EX,W secondary
    class GR,DR tertiary
    class CR output
```
...
```mermaid
flowchart LR
    S["Scoring"] --> E["Evolution"] --> G["Genealogy Recording"] --> R["LLM Reflection"]
    R --> M["Meta Control"] --> D["Diff Engine"] --> C["Coordinator"] --> H["Post-Generation Hooks"]

    classDef default fill:#1e1e2e,stroke:#e0af68
    classDef output fill:#1e1e2e,stroke:#50fa7b
    class S,E,G,R,M,D,C default
    class H output
```
...
```mermaid
flowchart TD
    S1["ScoreCache hit check\navoid repeated scoring"] --> S2["TieredScorer\nLLM budget gating"]
    S2 --> S3["Heuristic fallback\nwhen LLM is unavailable"]
    S3 --> S4["MemoryAwareScorer\nadjust scores based on\nexperiential evidence"]
    S4 --> S5["BatchScorer\nbatch pre-fill"]
...
```mermaid
flowchart LR
    TC["ToolCallRecord"] --> COL["ToolCallExperience\nCollector"] --> N["Normalizer"]
    N --> MES["MemoryExperience\nStore"] --> AE["AggregateEvidence"] --> EH["EvolutionHint"]

    classDef default fill:#1e1e2e,stroke:#e0af68
    classDef output fill:#1e1e2e,stroke:#50fa7b
    class TC,COL,N,MES,AE default
    class EH output
```
...
```mermaid
flowchart TD
    GO["Genome (old)"] --> DE["Diff Engine"]
    GN["Genome (new)"] --> DE
    DE --> RP["RuntimePatch[]"]

    classDef default fill:#1e1e2e,stroke:#e0af68
    classDef output fill:#1e1e2e,stroke:#50fa7b
    class GO,GN,DE default
    class RP output
```
